...
def test_collision_keeps_crowded_units_on_map_and_off_terrain() -> None:
    ...
    settings: MapSettings = {
        ...
        "Red": {"faction_id": "RUST", "peasant": [], "base": [[700, 700]], "ripper": [], "spitter": [], "brute": []},
        "Resources": {"wood": [], "gold": []},
        "Terrain": {
            "width": 800,
            "height": 800,
    ...
    simulation = HeadlessSimulation.from_settings(settings)
    manager = simulation.manager
    knights = manager.state.entities_by_content_id("guardian", team=TeamColor.BLUE)
...
def test_manager_public_attack_move_acquires_hostile_target() -> None:
    ...
    simulation = HeadlessSimulation.from_settings(settings)
    manager = simulation.manager
    knight = manager.state.entities_by_content_id("guardian", team=TeamColor.BLUE)[0]
    enemy = manager.state.entities_by_content_id("peasant", team=TeamColor.RED)[0]
...
def _wide_patrol_settings() -> MapSettings:
    ...
    return {
        ...
        "Red": {"faction_id": "RUST", "peasant": [], "base": [[760, 280]], "ripper": [], "spitter": [], "brute": []},
        "Resources": {"wood": [], "gold": []},
        "Terrain": {
            "width": 800,
            "height": 300,
...
def test_manager_public_return_cargo_order_deposits_resources() -> None:
    ...
    assert manager.teams[TeamColor.BLUE].resources["gold"] == 4
    simulation.close()
...
def test_manager_public_build_helper_reports_success() -> None:
    ...
    assert len(manager.state.entities_by_content_id("peasant", team=TeamColor.BLUE)) == 1
    ...
    assert len(manager.state.entities_by_content_id("peasant", team=TeamColor.BLUE)) == 2
...
def test_worker_constructs_spire_and_trains_arclight() -> None:
    ...
    spire = manager.state.entities_by_content_id("spire", team=TeamColor.BLUE)[0]
    ...
    assert len(manager.state.entities_by_content_id("arclight", team=TeamColor.BLUE)) == 1
...
def test_tower_auto_attacks_enemy_in_range() -> None:
    ...
    manager = simulation.manager
    tower = manager.state.entities_by_content_id("bastion", team=TeamColor.BLUE)[0]
    enemy = manager.state.entities_by_content_id("ripper", team=TeamColor.RED)[0]
...
def test_canceling_unfinished_house_refunds_and_removes_building() -> None:
    ...
    assert manager.state.entities_by_content_id("house", team=TeamColor.BLUE) == []
...
def test_manager_placement_helpers_place_selected_worker_building() -> None:
    ...
    houses = manager.state.entities_by_content_id("house", team=TeamColor.BLUE)
    assert len(houses) == 1
    assert houses[0].is_under_construction is True
    simulation.close()
...
def test_peasant_starts_harvesting_targeted_resources() -> None:
    ...
    peasant.state = "IDLE"
    gold = manager.state.resources_by_content("gold")[0]
...
def test_control_group_assign_and_recall_restores_selection() -> None:
    ...
    manager = simulation.manager
    guardians = manager.state.entities_by_content_id("guardian", team=TeamColor.BLUE)
    peasants = manager.state.entities_by_content_id("peasant", team=TeamColor.BLUE)

    manager.select_entities_for_team(TeamColor.BLUE, guardians)
    assert manager.assign_control_group(1) == 2

    manager.select_entities_for_team(TeamColor.BLUE, peasants)
    assert manager.recall_control_group(1) == 2
    assert set(manager.selected_entities) == set(guardians)
    simulation.close()
...
def test_select_units_like_selects_same_type() -> None:
    ...
    selected = manager.select_units_like(guardians[0])
