...
MINIMAP_HEIGHT: int = 150
MINIMAP_PADDING: int = 12
...
# RUST poison/toxin DoT: a poisoned target takes its ``poison_tick_damage`` once
# every ``POISON_INTERVAL`` frames until ``poison_remaining_frames`` runs out.
POISON_INTERVAL: int = 30
...
FOG_CELL_SIZE: int = 40

# Movement / formation / stuck-recovery tuning.
FORMATION_SPACING: int = 38
ATTACK_MOVE_MIN_ACQUIRE_RANGE: float = 160.0
STUCK_FRAME_LIMIT: int = 75
STUCK_PROGRESS_DISTANCE: float = 6.0
UNSTUCK_COOLDOWN_FRAMES: int = 45
